...
void RegionData::loadWorld(string filename) throw (const std::string&) 
{
  ...
  if (!fs::exists(worldFilename)) 
  {
    ...
    {
      loadWorldFilename = "";
      throw std::string("The world file " + worldFilename + " does not exist.");
    }
  }
  ...
  {
    loadWorldFilename = "";
    throw std::string("The image file " + imageFilename + " does not exist.");
  }
  ...
  if (!fs::exists(mapFilename)) 
  {
    ...
    {
      loadWorldFilename = "";
      throw std::string("The map file " + mapFilename + " does not exist.");
    }
  }
 ...
 *
**/
void RegionData::parseWorldFile(string filename) \
    throw (const std::string&) 
{
  ...
  {
    loadWorldFilename = "";
    throw std::string("Could not open log file " + filename + " for parsing.");
  }
...
cv::Mat RegionData::rotateImage(const cv::Mat& source, double angle, \
  ...
  data[0] = 0;  data[1] = 0;  data[2] = source.cols;  data[3] = source.cols;
  data[4] = 0;  data[5] = source.rows;  data[6] = 0;  data[7] = source.rows;
...
#ifdef P_PRINT_DEBUG
  cv::line(image,cvPoint(cx,cy),cvPoint(cx+100*v0x,cy+100*v0y),CV_RGB(0,255,0),2);
  cv::line(image,cvPoint(cx,cy),cvPoint(cx+50*v1x,cy+50*v1y),CV_RGB(0,255,0),2);
#ifdef ENABLE_IMSHOW
 ...
 * Author: Patrick Virie
 */
bool RegionData::solveQuadraticEquation(double A,double B,double C,double &r1, \
    double &r2) {
